...
function showCart(data) {
    if ($.trim(data) === '<h3>Корзина пуста</h3>') {
        $('#modalAddToCartProduct .modal-body').html(data).addClass('text-danger');
    } else {
        $('#modalAddToCartProduct .modal-body').html(data);
        $('#modalAddToCartProduct').modal('show');
    }


    if ($('#modalAddToCartProduct .count-price--cart').text()) {
        console.log($('#modalAddToCartProduct .count-price--cart').text());
...
$('body').on('change', '.aside input', function () {
    var chacked = $('.aside input:checked');
    var data = '';
    chacked.each(function () {
        data += this.value + ',';
    })
    if (data) {
        $.ajax({
            ...
            type: 'GET',
            beforeSend: function () {
                $('.loader-aside').fadeIn(300, function () {
                    $('.tt-product-listing').hide();
                });
            },
            success: function(res){
                $('.loader-aside').delay(500).fadeOut('slow', function(){
